/**
 * Saves one answer and updates the resume index. Idempotent per (answerSet, question).
 */
...
export async function POST(req: Request) {
  ...
  const aSet = await prisma.answerSet.findUnique({ where: { id: answerSetId }, include: { aeProfile: true } });
  ...
  // Compute updated saturation so the client can show the time estimate
  // and surface the "finish early" CTA the moment we have enough signal.
